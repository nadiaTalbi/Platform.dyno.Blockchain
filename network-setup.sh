...
function createOrgs() {
  ...
  . create-certificate.sh
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/dyno/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Dyno Identities"
    createDyno

    infoln "Creating Orderer Dyno Identities"
    createOrderer

  infoln "Generating CCP files for Dyno organizations"
