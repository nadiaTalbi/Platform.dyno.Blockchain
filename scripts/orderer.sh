...
# Create channel 
peer channel create -o localhost:7053 -c mychannel -f ./channel-artifacts/channel.tx --tls --cafile $ORDERER_CA
# To join orderer to a channel
osnadmin channel join --channelID mychannel --config-block ./channel-artifacts/genesis.block -o localhost:7053 --ca-file "$ORDERER_CA" --client-cert "$ORDERER_ADMIN_TLS_SIGN_CERT" --client-key "$ORDERER_ADMIN_TLS_PRIVATE_KEY" >> log.txt 2>&1